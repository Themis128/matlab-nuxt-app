...
test.describe('Recommendations API Integration', () => {
  ...
  test('should load recommendations page', async ({ page }) => {
    // Check page title
    await expect(page.getByRole('heading', { name: /recommendation|suggest/i })).toBeVisible({ timeout: 10000 })
  ...
  test('should display preference form with options', async ({ page }) => {
    ...
    const budgetInput = page.locator('input[placeholder*="budget"], input[placeholder*="price"], select[name*="price"]').first()

    // At least one price-related input should be present
    const hasPriceInput = await priceRangeInput.isVisible().catch(() => false) ||
    ...
    if (await preferencesSection.isVisible().catch(() => false)) {
      ...
      const hasOptions = await page.locator('input[type="checkbox"], input[type="radio"], select').count() > 0
  ...
  test.skip('should generate recommendations based on preferences', async ({ page }) => {
    ...
    const brandCheckbox = page.locator('input[type="checkbox"][name*="brand"], input[type="checkbox"][name*="company"]').first()
    ...
    const recommendButton = page.getByRole('button', { name: /recommend|find|suggest|search/i }).first()

    // Make sure button is enabled
    await expect(recommendButton).toBeEnabled({ timeout: 5000 }).catch(() => {})
    ...
    const hasRecommendations = await page.evaluate(() => {
      ...
      return pageText.includes('Models Found') ||
    ...
    const hasModelDetails = await page.evaluate(() => {
      const pageText = document.body.textContent || '';
      return ['ram', 'battery', 'screen', 'processor', 'camera', 'gb', 'mah', 'inches'].some(
        term => pageText.toLowerCase().includes(term)
      );
    });
  ...
  test('should filter recommendations when changing preferences', async ({ page }) => {
    ...
    const initialRecommendButton = page.getByRole('button', { name: /recommend|find|suggest|search/i }).first()

    // Make sure button is enabled
    await expect(initialRecommendButton).toBeEnabled({ timeout: 5000 }).catch(() => {})
    ...
    const initialCount = await page.locator('.card, .model-card, [class*="card"], div:has(> h3)').count()
    ...
    const filterRecommendButton = page.getByRole('button', { name: /recommend|find|suggest|search/i }).first()

    // Make sure button is enabled
    await expect(filterRecommendButton).toBeEnabled({ timeout: 5000 }).catch(() => {})
    ...
    const filteredCount = await page.locator('.card, .model-card, [class*="card"], div:has(> h3)').count()
    ...
    if (filteredCount === initialCount && initialCount > 0) {
      ...
      const priceElements = await page.locator('text=/\\$[0-9,]+|[0-9,]+\\s*USD/i').allTextContents()
  ...
  test('should allow clicking on a recommended model', async ({ page }) => {
    ...
    const firstModelCard = page.locator('.card, .model-card, [class*="card"], div:has(> h3)').first()
    ...
    if (cardVisible) {
      ...
      const hasContent = await page.evaluate(() => {
        return document.body.textContent?.includes('Models Found') ||
  ...
  test('should have working API connection', async ({ page, request }) => {
    ...
    try {
      const recommendResponse = await request.get('/api/dataset/models-by-price?price=500&tolerance=0.2')
