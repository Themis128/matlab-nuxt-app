...
test.describe.skip('Demo Page - Python API Integration', () => {
    ...
    if (await priceTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (inputCount > 0) {
        ...
        const predictButton = page
          .getByRole('button', { name: /predict|submit|calculate/i })
          .first()
        ...
        if (await predictButton.isVisible({ timeout: 5000 }).catch(() => false)) {
          ...
          const hasNewEndpoint = apiCalls.some(
            url => url.includes('/api/predict/price') || url.includes('/api/find-closest-model')
          )
  ...
  test('should make price prediction using new API', async ({ page }) => {
    ...
    if (await priceTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const responsePromise = page
        .waitForResponse(
          response => response.url().includes('/api/predict/price') && response.status() !== 404,
          { timeout: 10000 }
        )
        .catch(() => null)
  ...
  test('should make RAM prediction using new API', async ({ page }) => {
    ...
    if (await ramTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const responsePromise = page
        .waitForResponse(
          response => response.url().includes('/api/predict/ram') && response.status() !== 404,
          { timeout: 15000 }
        )
        .catch(() => null)
  ...
  test('should make battery prediction using new API', async ({ page }) => {
    ...
    if (await batteryTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const responsePromise = page
        .waitForResponse(
          response => response.url().includes('/api/predict/battery') && response.status() !== 404,
          { timeout: 10000 }
        )
        .catch(() => null)
  ...
  test('should make brand prediction using new API', async ({ page }) => {
    ...
    if (await brandTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const responsePromise = page
        .waitForResponse(
          response => response.url().includes('/api/predict/brand') && response.status() !== 404,
          { timeout: 10000 }
        )
        .catch(() => null)
      ...
      if ((await inputs.count()) > 0) {
        ...
        if (await predictButton.isVisible({ timeout: 5000 }).catch(() => false)) {
          ...
          const hasErrorComponent =
            (await page
              .locator(
                '.alert, .error, [class*="red"], [class*="orange"], [class*="alert"], [class*="warning"]'
              )
              .count()) > 0
          ...
          if (!hasErrorOrFallback && !hasErrorComponent && !hasStatusBadge) {
            ...
            const hasPriceDisplay =
              (await page.locator('text=/\\$[0-9,]+|[0-9,]+\\s*USD/i').count()) > 0
