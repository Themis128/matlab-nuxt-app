...
test.describe('Recommendations Page - Find Models by Price', () => {
  ...
  test('should search for models by price with default tolerance', async ({ page }) => {
    ...
    const modelCards = page
      ...
      .filter({ has: page.locator('text=/GB|mAh/i') })
  ...
  test('should filter results by brand', async ({ page }) => {
    ...
    const brandBadges = page
      .locator('[class*="cursor-pointer"]')
      .filter({ hasText: /Samsung|Apple|Xiaomi|OnePlus/i })
    ...
    if (brandCount > 0) {
      ...
      const filteredModelsText = await page
        .locator('text=/Models Displayed/i')
        .first()
        .textContent()
  ...
  test('should display model details in cards', async ({ page }) => {
    ...
    const firstCard = page
      .locator('[class*="hover:shadow"]')
      .filter({ has: page.locator('text=/RAM/i') })
      .first()
