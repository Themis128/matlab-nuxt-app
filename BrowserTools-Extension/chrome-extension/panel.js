...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'SERVER_VALIDATION_FAILED') {
    ...
    if (message.reason === 'connection_error' || message.reason === 'http_error') {
...
function cancelOngoingDiscovery() {
  if (isDiscoveryInProgress) {
    ...
    if (statusText && connectionStatusDiv && connectionStatusDiv.style.display === 'block') {
...
async function discoverServer(quietMode = false) {
  ...
  try {
    ...
    if (!hosts.includes(settings.serverHost) && settings.serverHost !== '0.0.0.0') {
