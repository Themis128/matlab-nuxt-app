...
function processJsonString(jsonString, maxLength) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      ...
      const processed = processArrayWithSizeLimit(parsed, settings.maxLogSize, (item) =>
        truncateStringsInData(item, maxLength, 0, 'root')
      );
...
async function sendToBrowserConnector(logData) {
  ...
  if (logData.type === 'network-request') {
    ...
    if (processedData.message) {
      ...
      processedData.message = processJsonString(processedData.message, settings.stringSizeLimit);
...
async function validateServerIdentity() {
  try {
    console.log(`Validating server identity at ${settings.serverHost}:${settings.serverPort}...`);
    ...
    if (!response.ok) {
      console.error(`Server identity validation failed: HTTP ${response.status}`);
...
function detachDebugger() {
  ...
  chrome.debugger.getTargets((targets) => {
    ...
    chrome.debugger.detach({ tabId: currentTabId }, () => {
      if (chrome.runtime.lastError) {
        console.warn('Warning during debugger detach:', chrome.runtime.lastError);
      }
      isDebuggerAttached = false;
...
const consoleMessageListener = (source, method, params) => {
  ...
  if (method === 'Runtime.exceptionThrown') {
    const entry = {
      type: 'console-error',
      message:
        params.exceptionDetails.exception?.description || JSON.stringify(params.exceptionDetails),
      level: 'error',
      timestamp: Date.now(),
    };
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          ...
          if (response && response.url) {
            console.log('Chrome Extension: Sending initial URL to server:', response.url);
      ...
      function tryFallbackGetUrl() {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
          ...
          if (tabs && tabs.length > 0 && tabs[0].url) {
            console.log('Chrome Extension: Got URL via fallback method:', tabs[0].url);
            ...
            console.warn('Chrome Extension: Could not retrieve URL through fallback method');
          }
        });
      }
    };
      ...
      try {
        ...
        if (message.type === 'heartbeat-response') {
          ...
          chrome.tabs.get(currentTabId, (tab) => {
            if (chrome.runtime.lastError) {
              console.error('Chrome Extension: Failed to get tab for screenshot:', chrome.runtime.lastError);
              ...
              (response) => {
                if (chrome.runtime.lastError) {
                  ...
                  if (retryCount < maxRetries) {
                    retryCount++;
                    console.log(`Retrying URL request (${retryCount}/${maxRetries})...`);
                  ...
                  ws.send(
                    JSON.stringify({
                      ...
                      tabId: chrome.devtools.inspectedWindow.tabId,
                      error:
                        'Failed to get URL from background: ' + chrome.runtime.lastError.message,
                      requestId: message.requestId,
                    })
                  );
                  return;
                  ...
                  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                    ...
                    ws.send(
                      JSON.stringify({
                        ...
                        url: url || null,
                        tabId: chrome.devtools.inspectedWindow.tabId,
                        error: response?.error || 'Failed to get URL from background',
                        requestId: message.requestId,
                      })
                    );
                  });
