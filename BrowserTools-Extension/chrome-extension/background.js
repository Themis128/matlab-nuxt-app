...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'UPDATE_SERVER_URL' && message.tabId && message.url) {
    ...
    updateServerWithUrl(message.tabId, message.url, message.source || 'explicit_update')
      .then(() => {
...
async function updateServerWithUrl(tabId, url, source = 'background_update') {
  ...
  chrome.storage.local.get(['browserConnectorSettings'], async result => {
    ...
    if (!success) {
      console.error(`Failed to update server with URL after ${maxRetries} attempts`)
    }
  })
}
...
async function retestConnectionOnRefresh(tabId) {
  ...
  chrome.storage.local.get(['browserConnectorSettings'], async result => {
    ...
    if (!isConnected) {
      console.log('Connection test failed after page refresh, initiating auto-discovery...')
    } else {
