      ...
      <UCard class="mb-8">
        ...
        <div class="space-y-8">
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-6">
              <label class="flex items-center gap-2 text-sm"
                ><input type="checkbox" value="price" v-model="selectedTypes" checked />
                Price</label
              >
              <label class="flex items-center gap-2 text-sm"
                ><input type="checkbox" value="ram" v-model="selectedTypes" checked /> RAM</label
              >
              <label class="flex items-center gap-2 text-sm"
                ><input type="checkbox" value="battery" v-model="selectedTypes" checked />
                Battery</label
              >
              <label class="flex items-center gap-2 text-sm"
                ><input type="checkbox" value="brand" v-model="selectedTypes" checked /> Company
          ...
          <div class="flex flex-wrap gap-4">
            <button @click="runPredictions" class="px-4 py-2 rounded-md bg-primary-500 text-white">
              Run AI Predictions
            </button>
            <button
              type="button"
...
const formValid = computed(() => {
  ...
  return Boolean(form.ram && form.battery && form.screen && selectedTypes.value.length > 0)
})
...
function runPredictions() {
  ...
  priceResult.value = Math.round(
    Number(form.ram || 0) * 95 +
      Number(form.battery || 0) * 0.12 +
      Number(form.screen || 0) * 40 +
      (form.company === 'Apple' ? 350 : 120)
  )
  ramResult.value = Math.max(
    2,
    Math.min(24, Math.round(Number(form.price || priceResult.value) / 120))
  )
