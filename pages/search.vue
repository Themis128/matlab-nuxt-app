      ...
      <div class="max-w-7xl mx-auto">
          ...
          <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
              ...
              <UFormGroup label="Brand(s)">
                <USelectMenu v-model=" filters.brands " :options=" availableBrands " multiple searchable
              ...
              <UFormGroup label="Price Range ($)">
                <div class="flex gap-2">
                  <UInput v-model.number=" filters.minPrice " type="number" placeholder="Min"
                    icon="i-heroicons-currency-dollar" />
                  <UInput v-model.number=" filters.maxPrice " type="number" placeholder="Max"
                    icon="i-heroicons-currency-dollar" />
                </div>
              </UFormGroup>
              ...
              <UFormGroup label="RAM (GB)">
                <div class="flex gap-2">
                  <UInput v-model.number=" filters.minRam " type="number" placeholder="Min" min="1" max="24" />
                  <UInput v-model.number=" filters.maxRam " type="number" placeholder="Max" min="1" max="24" />
                </div>
              </UFormGroup>
              ...
              <UFormGroup label="Battery (mAh)">
                <div class="flex gap-2">
                  <UInput v-model.number=" filters.minBattery " type="number" placeholder="Min" min="1000" />
                  <UInput v-model.number=" filters.maxBattery " type="number" placeholder="Max" max="10000" />
                </div>
              </UFormGroup>
              ...
              <UFormGroup label="Screen Size (inches)">
                <div class="flex gap-2">
                  <UInput v-model.number=" filters.minScreen " type="number" placeholder="Min" min="3" max="10"
                    step="0.1" />
                  <UInput v-model.number=" filters.maxScreen " type="number" placeholder="Max" min="3" max="10"
                    step="0.1" />
                </div>
              </UFormGroup>
              ...
              <UFormGroup label="Year(s)">
                <USelectMenu v-model=" filters.years " :options=" yearOptions " multiple searchable
                  option-attribute="label" value-attribute="value" placeholder="Select years" />
              </UFormGroup>

              <!-- Storage Range -->
              <UFormGroup label="Storage (GB)">
                <div class="flex gap-2">
                  <UInput v-model.number=" filters.minStorage " type="number" placeholder="Min" min="32" />
                  <UInput v-model.number=" filters.maxStorage " type="number" placeholder="Max" max="2048" />
                </div>
              </UFormGroup>
              ...
              <UFormGroup label="Model Name">
                <USelectMenu v-model=" filters.modelName " :options=" availableModels " searchable
                  :disabled=" filters.brands.length === 0 " placeholder="Select a model (choose brand first)" />
              </UFormGroup>

              <!-- Sort Options -->
              <UFormGroup label="Sort By">
                <USelect v-model=" sortBy " :options=" sortOptions " option-attribute="label" value-attribute="value" />
              </UFormGroup>
            ...
            <div class="flex gap-4">
              <UButton @click=" searchModels " :loading=" loading " color="primary" size="lg"
                icon="i-heroicons-magnifying-glass">
                Search
              </UButton>
              <UButton @click=" resetFilters " color="gray" size="lg" variant="outline" icon="i-heroicons-arrow-path">
                Reset
              </UButton>
            </div>
          </div>
        ...
        <div v-if=" results && !loading ">
            ...
            <div class="p-4">
              <div class="flex justify-between items-center">
                <div class="flex items-center gap-4">
                  <span class="font-semibold">{{ selectedModelsForComparison.length }} models selected</span>
                  <div class="flex gap-2">
                    <UButton @click=" compareSelectedModels " color="primary" icon="i-heroicons-scale" size="sm">
                      Compare Selected
                    </UButton>
                    <UButton @click=" clearModelSelection " color="gray" variant="outline" size="sm"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            <UCard v-for=" model in results.models " :key=" `${ model.company }-${ model.modelName }` "
              class="hover:shadow-lg transition-shadow" :class=" { 'ring-2 ring-primary': isModelSelected( model ) } ">
              <!-- Selection Checkbox -->
              ...
              <div class="p-4">
                ...
                <!-- Additional Specs -->
                <div v-if=" model.storage || model.processor "
          ...
          <!-- Pagination -->
          <div v-if=" results.pagination.hasMore || results.pagination.offset > 0 " class="flex justify-center gap-4">
            <UButton @click=" loadPreviousPage " :disabled=" results.pagination.offset === 0 " color="gray"
              variant="outline" icon="i-heroicons-arrow-left">
              Previous
            </UButton>
            <UButton @click=" loadNextPage " :disabled=" !results.pagination.hasMore " color="primary"
...
<script setup lang="ts">
  ...
  const availableBrands = [
    ...
    'Oppo', 'Vivo', 'Huawei', 'Sony', 'Motorola', 'Nokia',
    'Nothing', 'Asus', 'LG', 'Tecno', 'Infinix', 'POCO', 'Redmi'
  ]
  ...
  const fetchAvailableModels = async ( brands: string[] ) =>
  {
    ...
    try
    {
      ...
      console.log( 'Fetching models for brands:', brands, 'URL:', `http://localhost:8000/api/dataset/models-by-company?${ params }` )
      const models = await $fetch<string[]>( `http://localhost:8000/api/dataset/models-by-company?${ params }` )
  ...
  const toggleModelSelection = ( model: PhoneModel, selected: boolean ) =>
  {
    if ( selected )
    {
      if ( selectedModelsForComparison.value.length < 5 && !selectedModelsForComparison.value.includes( model.modelName ) )
      {
  ...
  useHead( {
    ...
    meta: [
      { name: 'description', content: 'Search mobile phones by multiple criteria including brand, price, RAM, battery, and more' }
    ]
  } )
</script>
