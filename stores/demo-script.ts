...
// Validation helpers
const validateNumber = (value: number | null | undefined, min: number, max: number): string | undefined => {
...
const priceValidation = computed(() => ({
  ...
  front_camera: priceInput.value.front_camera !== null ? validateNumber(priceInput.value.front_camera, 0, 100) : undefined,
  back_camera: priceInput.value.back_camera !== null ? validateNumber(priceInput.value.back_camera, 0, 200) : undefined,
  storage: priceInput.value.storage !== null ? validateNumber(priceInput.value.storage, 32, 2048) : undefined,
}))
...
const predictPrice = async () => {
  ...
  try {
    ...
    let response;
    try {
      response = await $fetch<{ price: number, error?: string, statusCode?: number }>('/api/predict/price', {
        method: 'POST',
        body,
      })
    ...
    // Show error message
    priceError.value = err.data?.message || err.message || 'Failed to predict price. Please try again.'
...
const predictBrand = async () => {
  ...
  try {
    ...
    brandError.value = err.data?.message || err.message || 'Failed to predict brand. Please try again.'
...
const predictRAM = async () => {
  ...
  try {
    ...
    ramError.value = err.data?.message || err.message || 'Failed to predict RAM. Please try again.'
...
const predictBattery = async () => {
  ...
  try {
    ...
    batteryError.value = err.data?.message || err.message || 'Failed to predict battery. Please try again.'
