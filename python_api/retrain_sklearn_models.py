...
def create_enhanced_features(df, target_col):
    ...
    if target_col == "ram_parsed":
        df_features["price_segment"] = pd.cut(
            df["price_parsed"],
...
def train_with_cross_validation(X, y, model_name, model_type="regression"):
    ...
    param_grids = {
        ...
        "svr": {"C": [0.1, 1.0, 10.0], "gamma": ["scale", "auto"], "kernel": ["rbf", "linear"]},
    }
...
def train_price_model(df_clean):
    ...
    scaler_dict = {"X_scaler": scaler, "y_scaler": scaler, "use_log": False}  # Use same scaler for both (simplified)
...
def train_ram_model(df_clean):
    ...
    actual_model = model.named_steps["model"]
    scaler = model.named_steps["scaler"]
    scaler_dict = {"X_scaler": scaler, "y_scaler": scaler, "use_log": False}
