...
class TestPricePrediction:
    ...
    def test_predict_price_with_invalid_inputs(self):
        ...
        # Very low RAM
        result = predict_price(
            ram=1, battery=3000, screen_size=5.0, weight=150, year=2020, company="Unknown", processor="Unknown"
        )
        assert result > 0

    @pytest.mark.parametrize(
        "ram,battery,expected_min",
        [
            (4, 3000, 100),
            (8, 5000, 200),
            (12, 6000, 300),
    ...
    def test_predict_price_scales_with_specs(self, ram, battery, expected_min):
        ...
        result = predict_price(
            ram=ram, battery=battery, screen_size=6.5, weight=200, year=2024, company="Samsung", processor="Snapdragon"
        )
...
class TestRAMPrediction:
    ...
    def test_predict_ram_correlates_with_price(self):
        ...
        low_price_ram = predict_ram(
            battery=4000, screen_size=6.0, weight=180, year=2024, price=200, company="Budget", processor="Basic"
        )

        high_price_ram = predict_ram(
            battery=5000, screen_size=6.5, weight=200, year=2024, price=1000, company="Premium", processor="Advanced"
        )
...
class TestBatteryPrediction:
    ...
    def test_predict_battery_returns_number(self):
        ...
        result = predict_battery(
            ram=8, screen_size=6.5, weight=200, year=2024, price=500, company="Samsung", processor="Snapdragon 8 Gen 2"
        )
        assert isinstance(result, (int, float))
        assert result > 0
...
class TestBrandPrediction:
    ...
    def test_predict_brand_returns_string(self):
        ...
        result = predict_brand(
            ram=8, battery=5000, screen_size=6.5, weight=200, year=2024, price=500, processor="Snapdragon 8 Gen 2"
        )
    ...
    def test_predict_brand_known_brands(self):
        ...
        result = predict_brand(
            ram=8, battery=5000, screen_size=6.5, weight=200, year=2024, price=500, processor="Snapdragon"
        )
...
class TestEdgeCases:
    ...
    def test_extreme_values(self):
        ...
        # Very high specs
        price = predict_price(
            ram=24, battery=10000, screen_size=8.0, weight=300, year=2025, company="Premium", processor="Latest"
        )
        assert price > 0
    ...
    def test_very_old_year(self):
        """Test with old year"""
        price = predict_price(
            ram=4, battery=3000, screen_size=5.0, weight=150, year=2015, company="Old", processor="Old"
        )
        assert price > 0

    def test_empty_strings(self):
        """Test with empty string inputs"""
        price = predict_price(ram=8, battery=5000, screen_size=6.5, weight=200, year=2024, company="", processor="")
        assert price > 0
