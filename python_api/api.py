...
try:
    ...
    try:
        from predictions_tensorflow import predict_battery, predict_brand, predict_price, predict_ram
...
async def predict_price_endpoint(request: PriceRequest):
    ...
    try:
        ...
        if (
            request.ram <= 6
            and request.battery <= 4500
            and request.screen <= 6.5
            and request.year <= 2022
            and request.company.lower() not in ["apple"]
        ):
            price = min(price, 999)
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=port, log_level="debug", access_log=True, reload=False)
