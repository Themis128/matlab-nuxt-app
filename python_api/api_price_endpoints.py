...
class PriceSearchRequest(BaseModel):
    ...
    preferred_source: Optional[str] = Field(None, description="Preferred API source: 'google', 'amazon', or None for all")
...
async def search_price(
    ...
    if not price_manager.is_any_configured():
        raise HTTPException(
            status_code=503,
            detail="No price APIs configured. Please set up API keys in environment variables."
        )
...
async def get_price_api_status():
    ...
    return {
        ...
        "price_comparison": {
            "api_key_set": bool(price_manager.price_comparison.api_key)
        },
        "any_configured": price_manager.is_any_configured()
    }
