...
class DataPreprocessor:
            ...
            if missing_count > 0:
                if col in fill_strategies:
                    ...
                    if strategy == 'mean' and pd.api.types.is_numeric_dtype(df[col]):
                        ...
                        fill_value = df[col].mode().iloc[0] if not df[col].mode().empty else default_fills.get('numeric' if pd.api.types.is_numeric_dtype(df[col]) else 'str', 'Unknown')
                    else:
                        fill_value = default_fills.get('numeric' if pd.api.types.is_numeric_dtype(df[col]) else 'str', 'Unknown')
    ...
    def _validate_data_types(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numeric_cols = self.config['numeric_validation']['price_columns'] + \
    ...
    def _engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if feature_config['create_storage_category'] and 'Storage' in df.columns:
            df['Storage Category'] = pd.cut(
                df['Storage'],
                bins=[0, 64, 128, 256, float('inf')],
                labels=['Basic', 'Standard', 'High', 'Ultra']
            )
