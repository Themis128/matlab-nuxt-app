% Back Camera Prediction Model Training
% This script trains a model to predict back camera MP from other phone specifications
% Input: RAM, Battery, Screen Size, Weight, Year, Price, Company
% Output: Back Camera MP
%
% Usage: run('train_back_camera_prediction_model.m')

fprintf('=== Back Camera Prediction Model Training ===\n\n');
...
% Filter rows with valid back camera data
validCamIdx = ~isnan(backCamera_clean) & backCamera_clean > 0;
fprintf('   Valid back camera samples: %d / %d\n', sum(validCamIdx), length(backCamera_clean));

if sum(validCamIdx) < 50
    error('Insufficient back camera data. Need at least 50 samples.');
end

% Features: RAM, Battery, Screen Size, Weight, Year, Price, Company
% Target: Back Camera MP
...
y = backCamera_clean(validCamIdx);  % Target: Back Camera MP

numFeatures = size(X, 2);
fprintf('   ✓ Features: %d samples × %d features\n', size(X, 1), numFeatures);
fprintf('   ✓ Back Camera range: %.0f - %.0f MP\n', min(y), max(y));
...
%% Step 6: Train
fprintf('\nStep 6: Training back camera prediction model...\n\n');
...
resultsPath = 'trained_models/back_camera_prediction_results.mat';
save(resultsPath, 'YPred', 'YTest_original', 'testIdx', ...
     'mse', 'mae', 'rmse', 'r2', 'mape');
fprintf('   ✓ Results saved to: %s\n', resultsPath);

fprintf('\n=== Back Camera Prediction Model Training Complete ===\n\n');
