...
> **Goal:** Split `mcp-server` into two packages for better modularity and cleaner publishing
...
4. [Detailed File Movement](#detailed-file-movement)
...
Split `mcp-server` into two packages:

| Package       | Purpose                                                                                | Publishing          |
| ------------- | -------------------------------------------------------------------------------------- | ------------------- |
| **mcp-core**  | Private workspace package with shared code (tools, API client, server builder)         | ❌ Never published  |
| **mcp-stdio** | Published package (keeping name `@sentry/mcp-server`) that fully bundles all core code | ✅ Published to npm |
...
| ------- | -------------------- | --------------------- |
| Core    | `@sentry/mcp-core`   | ❌ Private            |
| Stdio   | `@sentry/mcp-server` | ✅ Keep existing name |

### Build Strategy: DevDependency + Full Bundling
...
2. Update `package.json`:
   ...
   - Add `"private": true`
   - Remove `bin` field
   - Remove stdio-specific exports from `exports` field
3. Test build: `cd packages/mcp-core && pnpm build`
...
| ------------------------------ | ----------------------------------------------- |
| `CLAUDE.md`                    | Repository map section, package descriptions    |
| `docs/architecture.mdc`        | Package structure, dependencies diagram         |
| `docs/releases/stdio.mdc`      | New package structure, build process            |
| `docs/cursor.mdc`              | Package references                              |
| `AGENTS.md`                    | Package structure                               |
| `packages/mcp-core/README.md`  | Note it's private, explain shared code          |
| `packages/mcp-stdio/README.md` | Installation instructions, note it bundles core |
...
| Scenario          | Action                                               |
| ----------------- | ---------------------------------------------------- |
| Before committing | Delete mcp-stdio, rename mcp-core back to mcp-server |
| After committing  | Revert the commit: `git revert <commit-hash>`        |
| If published      | Publish a new patch version with the old structure   |
...
| Aspect           | Impact                              |
| ---------------- | ----------------------------------- |
| Package name     | ✅ No change (`@sentry/mcp-server`) |
| Installation     | ✅ Same: `npx @sentry/mcp-server`   |
| CLI flags        | ✅ Same behavior                    |
| Knowledge needed | ✅ No knowledge of mcp-core needed  |

### For Developers

| Aspect          | Impact                               |
| --------------- | ------------------------------------ |
| Code separation | ✅ Clear: core vs transport-specific |
| Cloudflare      | ✅ Can depend on core directly       |
| Modularity      | ✅ Better for future transports      |
| Type safety     | ✅ Full during development           |
| Build caching   | ✅ Turbo caching works correctly     |

### For the Codebase

| Aspect             | Impact                                        |
| ------------------ | --------------------------------------------- |
| mcp-core           | ✅ Private (never published)                  |
| mcp-stdio          | ✅ Fully self-contained                       |
| Package boundaries | ✅ Clean                                      |
| Maintainability    | ✅ Easier to maintain and extend              |
| Extensibility      | ✅ Future transports can be separate packages |
...
### Why not use tsup instead of tsdown?

tsdown is already used and works well. No need to change.

### Should we bundle node_modules too?

No, only mcp-core. External packages like MCP SDK and Sentry should remain external.

### What about source maps?

Enable in tsdown config for better debugging.

### How to handle version bumps?

Both packages should have same version. Consider using Changesets or manual sync.

### What about the README?

mcp-stdio keeps the user-facing README. mcp-core gets a developer-focused README.
...
| --------- | ---------------------------- | --------------- |
| 1-2       | Core rename + Stdio creation | 2-3 hours       |
| 3-4       | Update other packages        | 1 hour          |
| 5-6       | Build pipeline               | 1 hour          |
...
- [ ] `pnpm -w run build` succeeds for all packages
- [ ] `pnpm -w run test` passes all tests
- [ ] Can run stdio server: `node packages/mcp-stdio/dist/index.js --help`
- [ ] Can pack and install: `npm install -g ./packages/mcp-stdio/*.tgz`
- [ ] Cloudflare dev server works: `cd packages/mcp-cloudflare && pnpm dev`
- [ ] No `@sentry/mcp-core` in packed mcp-stdio `package.json`
