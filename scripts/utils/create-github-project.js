...
const PROJECT_CONFIG = {
  ...
  description: 'Track and manage development of deep learning models for mobile phone dataset analysis. Includes price prediction, RAM prediction, battery prediction, and brand classification models.',
...
function getOwnerAndType() {
  ...
  if (orgIndex !== -1 && args[orgIndex + 1]) {
    ...
    if (!/^[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9]$|^[a-zA-Z0-9]$/.test(owner)) {
      console.error('Error: Invalid owner name. Owner must be a valid GitHub username or organization name.');
      process.exit(1);
    }
  ...
  try {
    ...
    if (!/^https?:\/\/github\.com\/[a-zA-Z0-9\-_]+\/[a-zA-Z0-9\-_]+\.git$/.test(remoteUrl) &&
      !/^git@github\.com:[a-zA-Z0-9\-_]+\/[a-zA-Z0-9\-_]+\.git$/.test(remoteUrl)) {
      console.log('Warning: Git remote URL does not match expected GitHub pattern, skipping auto-detection.');
    } else {
