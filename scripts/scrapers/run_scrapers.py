...
class ScraperOrchestrator:
    ...
    def save_results(self, output_path: str = None):
        ...
        # SECURITY: Validate output path BEFORE creating Path object to prevent path traversal
        from pathlib import Path

        # Validate string input first
        if '..' in output_path:
            logger.error(f"Security: Path traversal detected in output path: {output_path}")
            return
        ...
        if not str(output_resolved).startswith(str(cwd)):
            logger.error(f"Security: Output file path outside working directory: {output_path}")
            return
